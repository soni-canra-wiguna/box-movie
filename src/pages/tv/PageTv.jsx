...
function PageTv() {
  document.title = "Box Movie | Tv"
  const [selectedGenres, setSelectedGenres] = useState([])
  const [isFilterButton, setIsFilterButton] = useState(false)

  // get genre
  const { data: genres, isLoading: loadingGenreTvPage } = useQuery({
    queryKey: ["genre tv"],
    queryFn: async () => {
      const { data } = await axios.get(
        `${import.meta.env.VITE_BASE_URL}/genre/${type.tv}/list?api_key=${
  ...
  const {
    ...
    queryKey: ["infinite scroll tv", genreIds],
    queryFn: async ({ pageParam = 1 }) => {
      const res = await axios.get(
        `${import.meta.env.VITE_BASE_URL}/discover/${type.tv}?api_key=${
          import.meta.env.VITE_API_KEY
  ...
  const content =
    ...
    data.pages?.map((page) =>
      page.results?.map((results) => (
        <CardGrid ref={ref} key={results.id} data={results} type="tv" />
      ))
    )
